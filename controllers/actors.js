...
const {Actor} = require('../db') //con esas llaves en Actor, solo extraemos aquel que se llama igual
//Ese Actor ya esta inicializado con el sequelize en el db
...
function create(req, res, next) {
    ...
    //LO que estoy diciendo es que me cree un Actor con ese nombre y ese apellido, 
...
function list(req, res, next) {
    Actor.findAll().then(objects => res.json(objects)).catch(ex => res.send(ex));
}

function index(req, res, next) {
    const id = req.params.id;
    Actor.findByPk(id).then(object => res.json(object)).catch(ex => res.send(ex));
}
...
function destroy(req, res, next) {
    ...
    Actor.destroy({ where: { id : id} }).then(object => res.json(object)).catch(ex => res.send(ex));
}
